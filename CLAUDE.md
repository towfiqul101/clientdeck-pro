...
- **A verified custom domain serves ONLY the portal surface** (Session 14 follow-up): Vercel attaches the domain to the whole project, so middleware step 0 redirects every non-portal path on a white-label host (`/`, `/login`, marketing pages, …) to `/portal` — before this, `portal.agency.com/` served the RoundTrack marketing site to the agency's clients. `/api/*`, `/sw.js`, and `/site.webmanifest` stay reachable (the portal's own fetches + PWA need them). Hosts that are *not* a verified custom domain keep normal routing on non-portal paths — deliberate, so a misconfigured `NEXT_PUBLIC_APP_URL` can't 404 the primary site.
