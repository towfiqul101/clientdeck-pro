...
export async function sendAgencyWelcomeEmail(
  ...
  const appUrl = process.env.NEXT_PUBLIC_APP_URL || "https://roundtrackpro.com";
  const subject = "Welcome to RoundTrack Pro — let's get you set up";
...
4. Install the RoundTrack Pro snapshot to load your pipelines and custom fields
...
3. Install the RoundTrack Pro snapshot to load your pipelines and custom fields
4. Add your first client and generate a dispute round`;
  const text = `Hi ${agency.owner_name || "there"},

Welcome to RoundTrack Pro! Here's how to get ${agency.name} up and running:

${steps}

Need a hand? Just reply to this email and we'll help you get set up.

— The RoundTrack Pro Team`;
